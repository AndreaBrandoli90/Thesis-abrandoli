...
\bigskip
\noindent
We assume that each pair of nodes is connected by a bi-directional reliable virtual link.
All the messages sent on this link are delivered, eventually, and in the order in which they were sent\footnote{More precisely, a sender keeps re-transmitting a message until it receives an acknowledgment or crashes}. A receiver can tell who sent a message, thus a message cannot be forged. Standing the above consideration, a message sent by a byzantine node is indistinguishable from a message sent by an honest node.

\bigskip
\noindent
As specified in a detailed article about the building blocks of consensus by Song and others \cite{Song:2008:BBC:1785854.1785862}, in the consensus problem nodes run \textit{actors}, that are either \textit{proposers}, each of which proposes a \textit{proposal}, or \textit{deciders}, each of which \textit{decides} one of the proposals. Assuming there exists at least one correct proposer
...
\bigskip
\noindent
Now that we have clear in mind what are the ingredients for the consensus recipe, a question bales to us: which are the algorithms that solve byzantine agreement? What type of validity they fulfill? The King algorithm is one of the best examples, but we have to restrict to the synchronous model. 
